...
void LevelBuilder::generateLayout(std::shared_ptr<Level> level) {
    ...
    for (Door &d : spawnRoomSettings->doors) {
        prepareNewRoom(roomMap, roomsToGenerate, roomsLeft, nextRoomId, spawnRoomSettings, d);
    }

    if (roomsToGenerate.size() == 0 && roomsLeft > 0) {
        // Create a new room
        forceNewRoom(level, roomMap, roomsToGenerate, roomsLeft, nextRoomId);
    }
    ...
    while (roomsToGenerate.size() > 0) {
        ...
        for (Door &d : r->doors) {
            ...
            if (d.destinationRoom == -1) {
                prepareNewRoom(roomMap, roomsToGenerate, roomsLeft, nextRoomId, r, d);
            }
        }
        if (roomsToGenerate.size() == 0 && roomsLeft > 0) {
            // Create a new room
...
void LevelBuilder::prepareNewRoom(std::map<std::pair<int, int>, std::shared_ptr<RoomSettings>>& roomMap, std::deque<std::pair<int, Door>>& roomsToGenerate, int& roomsLeft, int& nextRoomId, std::shared_ptr<RoomSettings> r, Door &d) {
...
void LevelBuilder::forceNewRoom(std::shared_ptr<Level> level, std::map<std::pair<int, int>, std::shared_ptr<RoomSettings>> &roomMap, std::deque<std::pair<int, Door>> &roomsToGenerate, int &roomsLeft, int &nextRoomId) {
    // Create a new room
